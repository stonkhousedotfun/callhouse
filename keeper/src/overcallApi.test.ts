 ...
 * generated 2026-09-13 (89 unique). SeaportOrderLib and ValoremLib are linked libraries, so their
 * reverts surface through the vault's calls; a decoder that only knew Vault.json's 54 would print
 * `BadFeeSplit` as an opaque selector.
 */
const PINNED_ERROR_SIGNATURES = [
  ...
  'BadFeeSplit(uint256,uint256,uint256,uint256)',
  ...
  'BadOfferer(address)',
  'BadOrderType(uint8)',
  'BadOvercallRecipient(address,address)',
  ...
  'NothingToClaim()',
  'OfferAmountZero()',
  'OfferExceedsInventory(uint256,uint256)',
  'OptionAssetMismatch(address,address)',
  'OptionExerciseAssetMismatch(address,address)',
  'OptionNotApproved(uint256)',
  'OptionNotInCurrentCycle(uint256,uint32,uint32)',
  'OptionWindowMismatch(uint40,uint40)',
  'OraclePaused()',
  'OrderHashMismatch(bytes32,bytes32)',
  'OtmBandInverted(uint16,uint16)',
  'OvercallFeeRoundsToZero(uint256,uint256)',
  ...
  'RegistryAssetMismatch(address,address)',
