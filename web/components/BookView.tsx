...
/**
 * The book for ONE market: every live 1-lot offer on the market's factory, the buyer's fill
 * (approve USDG to Seaport, then fulfillAdvancedOrder for one lot), and the calls this wallet
 * already holds on this market, with their exercise. In v2 mode, new v1 fills are hidden while
 * legacy exercise stays available. The factory
 * and the ticker are the market's; Seaport, the clearinghouse and USDG are shared (lib/contracts)
 * and the same for every market.
 */
...
export function BookView({ market }: { market: LegacyMarket }) {
          ...
          {liveRows.map(({ order, owner }) => {
            ...
            return (
              <Card key={`${order.offerer}-${order.salt.toString()}`}>
                <CardHead>
                  <CardTitle>
                    1 {market.ticker} · {fmtUsdg(ask, 3)} USDG
                  </CardTitle>
                </CardHead>
                <Rows>
