...
- [ ] W-20 DNS. `app.callhouse.finance` is a plain `CNAME`. **`callhouse.finance` is an apex**, and a `CNAME` at a zone apex is not valid DNS — it needs a provider offering `ALIAS`/`ANAME` or Cloudflare CNAME flattening. Prerequisite for W-19 being publicly reachable; attach the domains only after a deploy is healthy, so a DNS failure stays distinguishable from an application failure
