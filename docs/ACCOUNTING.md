...
The subtlest part of Callhouse. Read this before changing anything under `contracts/src/`.

Everything here is enforced by tests in `contracts/test/`; where a rule has a test, the test is
...
Both the contract (`Policy.splitPremium`) and the keeper (`keeper/src/seaport.ts`) implement this,
...
Asserted continuously by the stateful suite in `contracts/test/invariant/`.
