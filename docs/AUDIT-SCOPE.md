...
The document we hand to the external auditor (`tasks.md` E-05; the engagement itself is E-06).
...
It does not repeat the architecture or the accounting. Read [ARCHITECTURE.md](ARCHITECTURE.md)
...
enforced in bytecode, and the 2026-09-12 internal review) first. `ops/safes.md` is the role
topology with a grep-level proof of the guardian claim (§4 there, re-run against `27d502a` on
2026-09-12; see the note under the table in §2 below); `ops/addresses.json` is the address book.

Nothing is deployed to mainnet. Every `chains.4663.ours.*` entry in `ops/addresses.json` is
...
2. A configuration review of `contracts/script/Deploy.s.sol` and `contracts/script/Configure.s.sol`
   and of the role topology in `ops/safes.md`: the constants, the preflight, what the deployer
...
HEAD `27d502a83d72aabf28838c6926cc0b2f5043deb5` (2026-09-12 21:32 -0700); nothing under
`contracts/` differed from HEAD while it was written. The rule that matters: the pinned commit
will be tagged `audit-<date>`; `git status --porcelain` is empty at that tag (the scratch files
currently at the repository root, `l2b.html`, `page.html`, `rh_sitemap.txt`, are removed or
ignored before tagging); the auditor should reproduce every figure in §6 and §8 from that tag,
...
until deployment. Once hosted, the route handler `site/app/.well-known/security.txt/route.ts`
(L36–41) returns 404 on purpose until `NEXT_PUBLIC_SECURITY_CONTACT_EMAIL` is set and the site
rebuilt (`tasks.md` L-05; SECURITY.md §6 describes the same gap). The engagement uses a direct
channel agreed at kickoff, not that route.

**Access.** The repository is `callhouse` (this tree), delivered as read access to the GitHub
repository or a tarball of the tagged commit, to be confirmed. First command after checkout:
`git submodule update --init --recursive` (`contracts/lib/openzeppelin-contracts` and
`contracts/lib/forge-std` are submodules; nothing builds without them). The canonical ABI,
including every event and custom error, is `ops/abis/Vault.json`, generated from `contracts/out`
(§8 "ABIs flow one way"); the keeper's `keeper/src/abi.ts` is a hand transcription and is not
authoritative.
...
protocol fee capped at 20% in bytecode (10% at launch), credits the net to holders through a
per-share index, and settles a batched redeem queue.
...
a token. ARCHITECTURE.md §2 has the who-holds-what table; SECURITY.md §2 lists the bytecode
properties this rests on.

| Key | Holder | Can | Cannot |
|---|---|---|---|
| `DEFAULT_ADMIN_ROLE` | Admin Safe, Gnosis Safe 2-of-3 (`ops/safes.md` §1) | `setPolicy` inside the `Policy.validate` caps; `setFeeRecipient` (non-zero); `setDepositCap` (unbounded, can close deposits); `setMaxPriceAge` in [1 hour, 7 days]; `acceptValoremFee`; `haltWrites` and `unhaltWrites`; grant/revoke `KEEPER_ROLE` and `GUARDIAN_ROLE`; grant admin to a fourth address; renounce | Move any Stock Token or USDG (there is no admin-gated transfer in the vault); upgrade; rescue or sweep to an arbitrary address; take more than 20% of a harvest; sell inside 1% OTM; widen staleness past 7 days. **No timelock on any admin action.** Worst case: 20% of harvested USDG on filled weeks, never principal |
| `KEEPER_ROLE` | Hot EOA run by `keeper/` (`ops/safes.md` §2) | `rollOpen` (chooses the rung and size within policy); `approveListing` (proposes the whole Seaport order, at most 3 authorisations per cycle); `cancelListing`; `invalidateAllListings`; `rollClose` from `cycleExpiryTs` | Hold the option tokens or the claim; pay premium anywhere but the vault and Overcall's fee address; list above strike, below the policy floor, past `cycleExerciseTs`, or more than inventory; halt or unhalt; change any parameter; move a token. Worst case: a wasted week |
| `GUARDIAN_ROLE` | 1-of-1 key on separate hardware, different continent (`ops/safes.md` §3) | `haltWrites` (blocks `rollOpen` and `approveListing` only); `cancelListing`; `invalidateAllListings` (needs no order data) | `unhaltWrites` (the guardian can stop, never start); change parameters; block deposits, instant redemption, the queue, USDG claims, `lockBook` or `rollClose`; move a token. `ops/safes.md` §4 is the checkable proof |
| Fee recipient | Fee Safe (`ops/safes.md` §6) | Receive the protocol fee through the best-effort push in `rollClose` or the permissionless `sweepFee()` (both via `_tryPayFee`, Vault L970–989) | Holds no role; nothing else |
| Deployer | EOA running `Deploy.s.sol` | Fix every immutable at construction (asset, USDG, clearinghouse, registry, feed, Seaport, conduit key, zone, Overcall fee recipient); pass `SAFE_ADMIN` as `admin`, so the Safe is admin from block one | Anything after the constructor. A wrong immutable is unfixable without a redeploy. Renounce is only ever from an account that is not the sole admin (`ops/safes.md` §7) |
...
On the proof in `ops/safes.md` §4: it was re-derived against HEAD `27d502a` on 2026-09-12 (the
...
What each can do to us is in ARCHITECTURE.md §7 and SECURITY.md §3–§4; what we assume about
...
|---|---:|---|---|
| `contracts/src/Vault.sol` | 533 | The deployed contract: shares, deposits, phase machine, roll, harvest, redeem queue, admin | 36 functions declared here (20 state-changing, 16 view) plus the constructor, plus the inherited ERC-20, AccessControl, Distributor and adapter surfaces |
| `contracts/src/Distributor.sol` | 110 | Abstract ERC-20 base: the 1e27-scaled USDG accrual index, settle-on-transfer, claims, the balance checkpoint and the payout clamp | `claimUsdg`, `claimUsdgTo`, `claimableUsdg`, `usdgOwed`, 7 auto-getters; 6 internal hooks used by Vault |
| `contracts/src/AdapterValorem.sol` | 77 | Abstract base: the per-cycle short position on Valorem (option id, claim id, count), position views, ERC-1155 receiver | 5 public views, 2 ERC-1155 hooks, 4 getters; `_writeCalls`/`_redeemClaim` reachable only through `rollOpen`/`rollClose` |
| `contracts/src/AdapterSeaport.sol` | 111 | Abstract base: one authorised listing at a time, EIP-1271, cancel/counter bump, 3-per-cycle budget, the one-time ERC-1155 operator approval | `isValidSignature`, 5 immutable getters, 4 storage getters; internal mutators reachable only through Vault |
| `contracts/src/lib/ValoremLib.sol` | 79 | **Linked public library, DELEGATECALL.** Option-vs-cycle validation, approval sizing incl. the Valorem engine fee, `write`, `redeem`, three never-reverting position views | `writeCalls`, `redeemClaim` (DELEGATECALL only), `lockedAssets`, `claimedExerciseProceeds`, `contractsAssigned` (views) |
| `contracts/src/lib/SeaportOrderLib.sol` | 147 | **Linked public library, DELEGATECALL.** Field-by-field validation of the keeper's `OrderComponents`, `getOrderHash`, `validate`, hash-checked `cancel` | `approve`, `cancel` (DELEGATECALL only), `toParameters` (pure) |
| `contracts/src/Policy.sol` | 133 | Internal pure library, inlined: the hard caps, OTM band, premium floor, the 95/5 per-contract split, utilisation and count caps, fee split, oracle normalisation | None external; 12 internal pure functions and 10 constants |
| **Total** | **1,190** | | |
...
`SeaportOrderLib` at 2, `ValoremLib` at 3, both in `contracts/out/Vault.sol/Vault.json`
...
for `SeaportOrderLib` only, and `ops/addresses.json` has a `seaportOrderLib` slot but no
`valoremLib` slot. The keeper dry run (`keeper/DRYRUN.md`, "Two failures first") hit a stale
...
**Interfaces.** `contracts/src/interfaces/` (`IValoremClear.sol` 121 nSLOC, `ISeaport.sol` 79,
...
checkout (`contracts/lib` holds only `forge-std` and `openzeppelin-contracts`; there is no
`contracts/script/lib`). The header is corrected before pinning to say only that the file was
...
**Mocks.** `contracts/src/mocks/` (557 nSLOC: `MockClear`, `MockSeaport`, `MockRegistry`,
...
**Scripts, in scope for configuration review only.** `contracts/script/Deploy.s.sol` fixes
...
`ops/safes.md` §7. Please check: that no constant is wrong for chain 4663 (Appendix C), that
...
is charged on gross USDG inflow including strike proceeds on assignment, i.e. on converted
principal, not only on premium (`_accrueHarvest` L819–830;
`test_protocolFeeIsChargedOnStrikeProceedsNotJustPremium`, `VaultAssignment.t.sol:123`). That is
a product decision; please confirm it is intended and disclosed (§7, Appendix A item 1).
...
a live order (`ops/recon/R2-R9-seaport-order-shape.md`, `R3-overcall-api.md`,
`sample-overcall-order.json`). A change on Overcall's side makes every new listing
...
**Surface (internal, with the entry point that reaches each).** `validate` L103 (constructor
L296, `setPolicy` L1010–1014): `minOtmBps >= 100`, `maxOtmBps <= 2500`, `minOtm <= maxOtm`,
`minPremiumBps >= 10`, `maxUtilizationBps <= 10000`, `protocolFeeBps <= 2000`, `maxContractsCap
!= 0`. `launchDefaults` L120: `{300, 1200, 40, 9500, 1000, 50}`. `strikeBand`/`checkStrike`
L137/L148 (`rollOpen`): inclusive `[spot × (1 + minOtm), spot × (1 + maxOtm)]`, floor-rounded,
`SpotZero` on 0. `minPremium`/`checkPremium` L161/L171 (`approveListing`): gross `>= spot ×
contracts × minPremiumBps / 10000`, floor-rounded, on gross before Overcall's cut (so the
vault's net floor at launch is 0.38%, not 0.40%). `splitPremium` L200 and
`minListableUnitPrice` L215 (SeaportOrderLib). `maxContracts` L225 (unused in `src`; tests only)
and `checkContracts` L232 (`rollOpen`, re-derives the same formula inline): `1 <= N <=
maxContractsCap` and `N <= floor(idle × maxUtil / 10000 / 1e18)`. `splitHarvest` L247
(`_accrueHarvest`, every deposit checkpoint and `rollClose`): fee floored, `(0, 0)` on zero
gross. `normalizeSpot` L266 (`_spotUsdg`): `SpotZero` on `answer <= 0` or a result that rounds
to 0; rescales `feedDecimals → 6`. Constants L43–73: `BPS 10_000`, `MIN_OTM_FLOOR_BPS 100`,
...
Overcall's 5% per-contract rounding (mirrored from `ops/recon/R3-overcall-api.md`, not from any
...
`fee + net == gross`, `fee <= 20%`, zero fee on zero harvest; `normalizeSpot` never returns 0
...
`master` (`ops/recon/R10-address-audit.md`). Compiler recorded: solc 0.8.16, optimizer 200
...
(`ops/recon/R4-valorem-abi.md`, caveat 5 at line 519 and "Still UNRESOLVED after this pass"
item 3; `R10-address-audit.md` §UNRESOLVED, bullet "Byte-level equivalence of the deployed
...
registries are one compilation unit (`ops/recon/R12-overcall-discovery.md`). No audit of it is
...
`registry` key and must never be wired in (`Deploy.s.sol`, `ops/addresses.json`
...
role itself is 10800 s (3 h) (`ops/abis/USDG.json` L45–46, `ops/recon/R6-stock-token.md` §7.3).
...
source (`R10-address-audit.md` §UNRESOLVED, "USDG upgrade authority mechanics"; Appendix B).
...
cite it as background only. Our ABI (`ops/abis/StockToken.json`) was recovered from the
...
*Freeze and blocklist surface* (`ops/recon/R6-stock-token.md`, all read live 2026-09-12 with
...
weekend gaps up to 78 h; `ops/recon/R5-price-feed.md`). Used only as a write gate and a UI
value, never in settlement. No sequencer uptime feed exists on 4663 (`ops/addresses.json`
...
**OpenZeppelin Contracts v5.7.0** (`contracts/lib/openzeppelin-contracts`, submodule
`cab19933`): `ERC20`, `AccessControl`, `ReentrancyGuard`, `SafeERC20`, `Math`, `IERC20`. Standard.
**forge-std v1.16.2** (`bf647bd`): tests only.

**Keeper, indexer, web** (`keeper/`, `indexer/`, `web/`, `site/`). No money authority: no key
they hold can move a token, and the vault re-validates every field the keeper proposes
(SECURITY.md §1, ARCHITECTURE.md §2). Out of scope. One exception worth a glance, not an
audit: the keeper's listing-construction path is the concrete example of what the vault must
reject. `keeper/src/seaport.ts` (`buildOrderComponents`, `splitPremium`, `localOrderHash`,
`localOrderDigest`, `PLACEHOLDER_SIGNATURE`), `keeper/src/policy.ts` (`pickWrite`,
`strikeBand`, `maxContracts`, `minUnitPrice6`) and `keeper/src/roll.ts` (`tick`, `reconcile`,
...
the vault, not the keeper. `contracts/test/unit/SplitDiff.t.sol` is the differential test that
...
Line numbers are at HEAD `27d502a`.

| # | Property | Where |
|---|---|---|
| P-01 | No `policy` value outside the hard caps (`minOtmBps >= 100`, `maxOtmBps <= 2500`, `minOtm <= maxOtm`, `minPremiumBps >= 10`, `maxUtilizationBps <= 10000`, `protocolFeeBps <= 2000`, `maxContractsCap != 0`) can ever be stored | `Policy.sol` L103–117; `Vault.sol` L296, L1010–1014 |
...
| P-07 | No state of `feeRecipient` or of USDG can make `rollClose` revert through the fee leg; `sweepFee()` always pays the stored recipient, never the caller | `Vault.sol` L857, L961–989 |
...
| P-12 | The phase machine moves only as drawn in ARCHITECTURE.md §3; `Idle ⇒ contractsWritten == 0 && claimKey == 0` | `Vault.sol` L676, L731, L769–770, L783–784, L791, L806; `invariant_phaseSanity` |
...
| P-25 | Governance cannot reach principal by any path, including `setFeeRecipient` plus a 20% fee on a future assigned week, `setDepositCap`, `setPolicy`, role grants, or renouncing | `Vault.sol` L996–1049 |

The money invariants of ACCOUNTING.md §7, asserted by the stateful suite
(`contracts/test/invariant/VaultInvariant.t.sol`, 64 runs × depth 600):
...
6. **Harvest is any USDG balance increase.** Vault L819–830. Donated USDG, strike proceeds and
   premium are all fee'd at `protocolFeeBps` and distributed. Try: a donation that distorts the
   index, the fee, or a deposit-then-queue sequence that captures premium landing between the
   last checkpoint and exercise; confirm a late depositor in `Listed` who shares assignment
...
16. **Admin without a timelock, and immutability.** No upgrade, no rescue, no timelock;
    ...
    premium floor, 100% utilisation leaving nothing idle for the queue, a 20% fee. Confirm P-25.
...
the review (`tasks.md` "Defects found and fixed during the build"; the review-era ones are
SECURITY.md §4 "Fixed" rows 1–5, where `tasks.md` #11 merges SECURITY.md #2 and #3):
...
Regression tests for 10–13 are the eight tests in `contracts/test/unit/VaultSecurity.t.sol`
...
| `test/unit/VaultAssignment.t.sol` | 16 | Full/partial/zero assignment, fee on strike proceeds, late depositor, queued redeemers through assigned weeks, fuzzed collateral/strike exactness |
...
`contracts/README.md` line 51 says 328; 307 is the measured figure and the one in `tasks.md`.
...
**Fork tests against live chain 4663** (`FOUNDRY_PROFILE=fork`, `contracts/test/fork/ForkLive.t.sol`,
...
**Keeper dry run** (`keeper/DRYRUN.md`, recorded 2026-09-13T05:49:32Z, anvil fork of chain
...
action or guardian `rollClose` ran.
...
validator (§7; `tasks.md` L-04). Apart from the single exercise in the keeper dry run above, no exercise
...
Actions run currently dies `startup_failure` at the account level (`tasks.md` L-01), so CI has
...
Open questions being closed before launch (`tasks.md` "Open questions to settle before launch";
SECURITY.md §5):
...
2. **Keeper prices at exactly the policy floor.** The vault re-reads spot at `approveListing`;
   ...
   `max(previous, live policy floor)`, `tasks.md` K-17.)
...
- **No upgradeability, no rescue function, no timelock on the Admin Safe.** A real bug means
  ...
  20%` on future harvests plus choosing `feeRecipient`.
- **The protocol fee is charged on gross USDG inflow, including strike proceeds on assigned
  weeks.** On a fully assigned 10-contract week at a $231 strike with $2.00 premium the fee is
  232.9 USDG of a 2,329 USDG harvest (`VaultAssignment.t.sol` header). The code and its tests
  treat this as intended; ACCOUNTING.md §6 currently says "both [fees] are on the premium
  only" and will be corrected. Please state whether you consider the disclosure adequate.
- **Any USDG that lands in the vault is harvest.** Donations are fee'd and distributed to
  holders.
...
`contracts/foundry.toml`: `solc_version = "0.8.28"`, `evm_version = "cancun"`, `optimizer =
...
forge-std v1.16.2 as git submodules under `contracts/lib/` (`contracts/foundry.lock`).
...
(`keeper/DRYRUN.md`), so an anvil fork that runs for more than a few minutes needs an archive
...
Deploy and verify (`contracts/README.md` "Deploying"; `ops/deploy.md`; `ops/safes.md` §7):
...
missing `Referer`; `ops/bsproxy.js` injects one.

Traps (`tasks.md` "Build constraints"):
...
- **Tag space.** Each unit suite deploys the whole fixture; with via-IR on, another
  ...
  `BaseTest` (`contracts/test/Base.t.sol`) rather than repeating them.
...
- **Stale artefacts.** A `Vault.json` compiled with a library pinned to a placeholder address
  ...
  (`keeper/DRYRUN.md`). A clean `forge build` against the committed `foundry.toml` has both
  libraries in `linkReferences` and `metadata.settings.libraries == {}`.
- **ABIs flow one way**: `contracts/out` → `ops/abis/Vault.json` → generated copies in
...
1. Per finding: title, severity, location (`file:line` at the pinned commit), description,
   ...
   `contracts/test/Base.t.sol` where feasible; the mocks in `contracts/src/mocks/` are the
...
(`tasks.md` L-07); the exact path is to be confirmed (Appendix B). Until then the site and
...
(`tasks.md` E-07). Its scope will mirror §3 and reuse the severity table above; the disclosure
...
1. **ACCOUNTING.md §6** says "Two fees stack, and both are on the premium only." The protocol
   fee is charged on the whole USDG balance delta, which includes strike proceeds on assigned
   weeks (`Vault.sol` `_accrueHarvest` L819–830, whose NatSpec says so;
   `test_protocolFeeIsChargedOnStrikeProceedsNotJustPremium`, `VaultAssignment.t.sol:123`).
   README.md line 164, "10% of USDG harvested (filled weeks only)", is the accurate wording.
2. **contracts/README.md line 51** says "328 unit and invariant tests"; the measured count and
   `tasks.md` say 307.
3. **`tasks.md`** lists 13 defects; **SECURITY.md §4** has 5 review rows. `tasks.md` #11 merges
   SECURITY.md #2 (tenor cap) and #3 (window mismatch).
4. **README.md line 165** says the launch deposit cap is "20–50 NVDA"; `Deploy.s.sol` hard-codes
   `LAUNCH_DEPOSIT_CAP = 20e18` and `tasks.md` L-07 says 20.
5. **README.md line 96** says SECURITY.md covers "audit scope". This file is the audit scope.
6. **`ops/addresses.json`** `chains.4663.ours` has a `seaportOrderLib` slot and no `valoremLib`
...
8. **ARCHITECTURE.md §3** ends "Pause and halt block `rollOpen` only"; `writesHalted` gates
   ...
   `contracts/README.md` L117 ("A halt blocks `rollOpen` **only**"). The checks at L677 and L732
   are the spec; the two NatSpec lines and the README line are corrected before the engagement
   commit is pinned (Appendix B). SECURITY.md and `ops/safes.md` §3 have it right.
...
- Housekeeping before the tag: correct the `writesHalted` NatSpec at `Vault.sol`
  L115 and L995 and `contracts/README.md` L117 to say `rollOpen` and `approveListing` (Appendix A
  ...
  re-derive every line number in this document and in `ops/safes.md` §4 at the tag.
- Site hosting at `callhouse.xyz`: the domain currently serves a registrar parking redirect,
  not the `site/` build, so the RFC 9116 path does not exist yet (§1).
- The public disclosure address (`NEXT_PUBLIC_SECURITY_CONTACT_EMAIL`, `tasks.md` L-05) and the
...
- Whether Zellic's August 2023 patch review covers the whole `ed53af23 → fe39eb73` delta of
  ...
  external review after the rename (§4; `ops/recon/R4-valorem-abi.md` caveat 5). We have not
...
- Source match of the live USDG implementation `0x68184c449e1a8f34fa18d289737129fd27b66f8f` to
  ...
  `defaultAdmin` can actually perform the UUPS upgrade (`ops/recon/R10-address-audit.md`
...
- Any Robinhood-commissioned audit of the Stock Token contracts (none found), and the contents
  ...
  `ops/recon/R6-stock-token.md` (live reads and state-override proofs, 2026-09-12); R6 proves
...
- Whether the protocol fee on strike proceeds is the intended and disclosed economic design
  (§7; Appendix A item 1).
...
- CI green on GitHub (`tasks.md` L-01); every figure here is from a local run.
- The forge build the auditor reproduces with (§8).

## Appendix C. Addresses on chain 4663

All rows reconfirmed 2026-09-12 by `ops/recon` (`ops/addresses.json`, `_reconfirmedAt
...
1200`, `minPremiumBps 40`, `maxUtilizationBps 9500`, `protocolFeeBps 1000`, `maxContractsCap
