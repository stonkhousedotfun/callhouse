...
src/targets.ts   one POST per target, with a deadline. Never throws, never logs a URL.
...
|---|---|---|
| `RELAY_TOKEN` | yes | Shared secret the keeper presents. ≥ 32 characters. `openssl rand -hex 32` |
...
| 400 | body is not JSON, or not a keeper alert (`issues` says which field) | retried, and will keep failing — read the relay log |
...
code (`http_429`, `timeout`, `ECONNREFUSED`), never a message or a URL.
...
truncated when formatted, never refused. `source`, `vault`, `chainId`, `at` and `data` are shown
when present.
...
🔴 error, 🟠 warn, 🔵 info. When a message is over the limit, `data` is truncated first (with
