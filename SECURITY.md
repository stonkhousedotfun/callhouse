...
Read [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) §2 for the trust boundaries and
[docs/ACCOUNTING.md](docs/ACCOUNTING.md) for the money maths. The per-alert response runbooks
are in `ops/alerts.md`.
...
is fixed and carries a regression test in `contracts/test/unit/VaultSecurity.t.sol`, which
documents each attack in full.

This was an internal review, not an external audit. Engaging one is still on the plan
(`tasks.md` E-05/E-06), along with the fork rehearsal weeks.
...
- **Error/ABI refresh.** The keeper was missing 32 custom-error fragments (it would have shown
  ...
  copies predated the fixes. All regenerated; the web ABI now has a committed generator
  (`web/scripts/gen-abis.mjs`) like the indexer's.
...
Tracked in `tasks.md` "Open questions":
...
`site/lib/legal.ts`; until that variable is configured and the site rebuilt, no address is
designated, `security.txt` returns 404 on purpose, and the legal page says so in words. Setting
it is a launch step in `ops/launch-legal.md`.
