...
const ACCESS_ROLES_SRC = path.join(opsAbisV2, "roles.json");
const ACCESS_ROLES_OUT = "lib/v2/accessManagerRoles.generated.ts";
 ...
 * COPIED TABLE: keep identical in web/scripts/gen-abis.mjs and keeper/scripts/gen-abis.mjs.
 */
...
const accessRoles = JSON.parse(readFileSync(ACCESS_ROLES_SRC, "utf8"));
emit(
  ACCESS_ROLES_OUT,
  `// GENERATED by scripts/gen-abis.mjs from ops/abis/v2/roles.json. Do not edit by hand.\n` +
    `// The production image contains indexer/ only, so AccessManager operation labels need this copy.\n` +
    `export const V2_ACCESS_MANIFEST = ${JSON.stringify(accessRoles, null, 2)} as const;\n`,
  ` (${Object.keys(accessRoles.roles).length} roles, ${Object.keys(accessRoles.targets).length} targets)`,
);
