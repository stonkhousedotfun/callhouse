...
|---|---|
| §1 `phase()`, `isStranded()`, `writesHalted()` | `week()` on the factory: `(id, strikeUsdg, exerciseTs, baseExpiryTs, askUsdg)`. A week whose `baseExpiryTs` is in the past is over and the keeper sets the next one; there is no phase and no strand. `writesHalted()` must be `false`. `pendingCount()` is the work waiting |
| §2 the oracle | this market's own `$FEED` (`latestRoundData`, `decimals` 8) and `$ASSET "oraclePaused()(bool)"`; `maxPriceAge()(uint32)` on the factory (4 days). Every equity feed sleeps at the weekend the way `RHNVDA / USD` does |
| §3 the Valorem fee switch | `$CLEAR "feesEnabled()(bool)"` and `$FACTORY "valoremFeeAccepted()(bool)"`. The Clear is **shared**: one flip stops `list` and every fill on every market at once |
| §4 choose the strike and the window | the keeper's per-market dry run: `pnpm --filter @callhouse/keeper solo:quote` with `ops/keeper/markets/$T.env` loaded prints strike, ask, window and the mode's evidence without sending. Band and floor: `$FACTORY "policy()(uint16,uint16,uint16,uint16,uint16,uint64)"`. `vol` markets take the 0.15-delta strike from Cboe's chain for **this** ticker; `fixed` (SGOV) takes spot + 5%. The ask is never below `minAskUsdg6` (0.10 USDG) |
| §5 create the option type | not a step: each account creates its own type inside `list` (`expiry = baseExpiryTs + index`) |
| §6 arm (`rollOpen`) | `setWeek(strike6, exerciseTs, baseExpiryTs, ask6)` by **this market's** keeper (`KEEPER_ROLE`): `cast send $FACTORY "setWeek(uint256,uint40,uint40,uint256)" <strike6> <exerciseTs> <baseExpiryTs> <ask6> --rpc-url $RH_RPC --private-key "$KEEPER_PK"`. Factory checks: exercise > now + 1 h (strict — exactly `now + 3600` reverts `BadWeek`), base expiry ≥ exercise + 1 day, `ask ≤ strike`. Verify with `week()`. Nothing is written |
| §7 authorise the listing | per pending account: `A=$(cast call $FACTORY "pendingAt(uint256)(address)" 0 --rpc-url $RH_RPC)`, `O=$(cast call $A "owner()(address)" --rpc-url $RH_RPC)`, then `cast send $FACTORY "listFor(address)" $O --rpc-url $RH_RPC --private-key "$KEEPER_PK"` (or the owner calls `list()` on the account). The account posts `requestedLots` FULL 1-lot orders and pins strike, ask, exercise and its own expiry. Verify: `liveCount()`, `liveAt(i)`; on the account `listedLots()`, `listedStrikeUsdg()`, `listedAskUsdg()`, `listedExerciseTs()`, `listedExpiryTs()`, `liveListingCount()` |
| §8 confirm a fill from our page | `app.stonkhouse.fun/<ticker>/book` (lowercase). The account's `lotOrder(salt)` view returns the exact Seaport components each lot is listed with; the fill is `fulfillOrder` with an empty signature and the account as zone. The hook re-checks the band floor and the premium floor at live spot, as the vault's did |
| §9 while listed, hourly | `liveCount()`; per live account `contractsWritten()(uint112)` (== lots sold), `claimKey()(uint256)` (non-zero after the first fill), `LotFilled(orderHash, optionId, premiumUsdg)` logs on the account; `keeper-<ticker>` `/health` |
| §10 close the book | nothing to lock: each order's end time is the account's `listedExerciseTs`, so fills stop there by themselves. `settle()` after `listedExpiryTs` is `close-week.md` |
